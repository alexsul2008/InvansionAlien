...
class InvasionAlien:
	...
	def __init__(self):
		...
		# self.bg = pygame.image.load(os.path.join(sourceFileDir, 'images', 'bg.jpg'))

		self.settings.screen_height = self.screen.get_rect().height
		pygame.display.set_caption("Инопланетное вторжение")
		#Создание экземпляра для хранения игровой статистики
		self.stats = GameStats(self)
		self.bs = BoardScore(self)
		self.ship = Ship(self)
	...
	def _create_alien(self, alien_number, row_number):
			...
			alien.x = allien_width + 2.3 * allien_width * alien_number
			alien.rect.x = alien.x
			alien.rect.y = 20 + alien.rect.height + 1.4 * alien.rect.height * row_number		# умножить на 2
